...
# Author: ChatGPT
# Purpose: Resume-quality EDA with Missing Value Handling, Outlier Detection,
# Inferential Statistics, Probability Distributions, and Business Insights
# =====================================================
...
for c in num_cols:
    ...
    print(f"{c}: Q1={q1:.2f}, Q3={q3:.2f}, IQR={iqr:.2f}, Outliers={outliers.shape[0]}")
...
# Normal Distribution: Sales & Profit
if 'sales' in df.columns:
...
# Binomial: Profitable vs Loss-making Orders
if 'profit' in df.columns:
    df['profitable_flag'] = np.where(df['profit'] > 0, 1, 0)
    p_hat = df['profitable_flag'].mean()
    print(f"\nBinomial Model: Success = Profitable Order, p̂ = {p_hat:.3f}")
...
if "ship_mode" in df.columns and "sales" in df.columns:
    ship_sales = df.groupby("ship_mode")["sales"].mean().sort_values(ascending=False)
    print("\nAverage Sales per Order by Ship Mode:")
    print(ship_sales)
    sns.barplot(x=ship_sales.index, y=ship_sales.values, palette="Set2")
    plt.title("Avg Sales per Order by Ship Mode")
    plt.show()
...
if "discount" in df.columns and "profit" in df.columns:
    ...
    plt.show()
    print("\nInsight: Higher discounts often lead to losses.")
...
if "order_date" in df.columns and "sales" in df.columns:
    ...
    plt.xlabel("Month")
    plt.show()
    print("\nInsight: Sales show seasonal peaks and troughs.")

print("\nEDA Completed Successfully with Inferential Statistics and Business Insights!")